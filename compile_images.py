...
def draw_map(file_name, json_path, save_path, counter):
    print("Printing image ", counter)
    ...
    if not os.path.exists(save_path):
        os.mkdir(save_path)
    #cv2.imwrite(save_path + "/" + "image" + str(counter) + '.png', cnt_map)

    #Save map through matplotlib to crop image and normalise resolution
    fig_map = plt.gcf()
    plt.clf()
    plt.imshow(cnt_map,cmap="gray")
    plt.draw()
    plt.axis('off')
    plt.pause(0.001)
    fig_map.savefig("cleaned_images/image"+str(counter)+".png")
...
if __name__ == '__main__':
    ...
    for map_id in map_ids:
        counter = counter + 1
        draw_map(map_id, json_path, save_path, counter)
